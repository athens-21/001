...
import {
    ...
    BarChart3, PieChart, Calendar, Filter, Edit, Trash2
} from 'lucide-react';
import {
...
export default function Dashboard() {
    ...
    const [currentStatsIndex, setCurrentStatsIndex] = useState(0); // Stats bar pagination
    ...
    const journalAnalytics = useMemo(() => {
        ...
        if (closedTrades.length === 0) {
            return {
                ...
                dailyReturn: 0, returnOnWinners: 0, returnOnLosers: 0,
    ...
    const loadData = async (isBackgroundRefresh = false) => {
        ...
        try {
            ...
            console.log('📊 Data refreshed:', tradesRes.data.length, 'trades loaded for account:', user.accountNumber);
        } catch (error) {
    ...
    const visibleKPIs = kpis.slice(currentKPIIndex, currentKPIIndex + 3);
    ...
    const calculateRiskRewardData = () => {
        ...
        return closedTrades.map(trade => {
            ...
            // Calculate R:R ratio
            // R:R = Reward / Risk = Profit / Estimated Risk
            // Estimate risk as average percentage or use 1:1 to 1:3 range
            const avgRisk = Math.abs(profit * 0.5); // Simplified: assume risked half of actual P/L
            const reward = Math.abs(profit);
            const rr = avgRisk > 0 ? Number((reward / avgRisk).toFixed(2)) : 1;
            
            // Limit R:R to reasonable range (0.1 to 10)
            const clampedRR = Math.min(Math.max(rr, 0.1), 10);
            
            // Win rate for scatter: 100 if win, 0 if loss
            // But we'll use a spread for better visualization
            const baseWinRate = isWin ? 75 : 25; // Base position
            const spread = (Math.random() - 0.5) * 20; // Add some spread for visibility
            const winRate = Math.max(0, Math.min(100, baseWinRate + spread));
    ...
    const calculatePerformanceHistogram = () => {
        ...
        filteredTrades.filter(t => t.status === 'CLOSED').forEach(trade => {
            ...
            dailyPL[date] = (dailyPL[date] || 0) + Number(trade.net_profit || 0);
        });
        
        return Object.entries(dailyPL).map(([date, pl]) => ({
            date,
    ...
    const statsItems = [
        {
            ...
            value: stats?.totalTrades || 0,
            subtitle: 'Total',
            colorClass: 'text-bloomberg-accent'
        },
        {
            title: 'Win Rate',
            value: `${stats?.winRate || 0}%`,
            subtitle: `${winLossData.wins}W / ${winLossData.losses}L`,
            colorClass: 'text-bloomberg-accent'
        },
        {
            title: 'P/L',
            value: `$${Number(stats?.totalProfitLoss || 0).toFixed(2)}`,
            subtitle: 'Net',
            colorClass: stats?.totalProfitLoss >= 0 ? 'text-bloomberg-success' : 'text-bloomberg-danger'
        },
        {
            title: 'Cumulative',
            value: `$${journalAnalytics.accumulativeReturnNet.toFixed(2)}`,
            subtitle: `${journalAnalytics.accumulativeReturnPercent.toFixed(1)}%`,
            colorClass: journalAnalytics.accumulativeReturnNet >= 0 ? 'text-bloomberg-success' : 'text-bloomberg-danger'
        },
        {
            title: 'Daily',
            value: `$${journalAnalytics.dailyReturn.toFixed(2)}`,
            subtitle: 'Today',
            colorClass: journalAnalytics.dailyReturn >= 0 ? 'text-bloomberg-success' : 'text-bloomberg-danger'
        },
        {
            title: 'Weekly',
            value: `$${journalAnalytics.weeklyReturn.toFixed(2)}`,
            subtitle: '7D',
            colorClass: journalAnalytics.weeklyReturn >= 0 ? 'text-bloomberg-success' : 'text-bloomberg-danger'
        },
        {
            title: 'Monthly',
            value: `$${journalAnalytics.monthlyReturn.toFixed(2)}`,
            subtitle: '30D',
            colorClass: journalAnalytics.monthlyReturn >= 0 ? 'text-bloomberg-success' : 'text-bloomberg-danger'
        },
        {
            title: 'Winners',
            value: `$${journalAnalytics.returnOnWinners.toFixed(2)}`,
            subtitle: 'Total',
            colorClass: 'text-bloomberg-success'
        },
        {
            title: 'Losers',
            value: `$${journalAnalytics.returnOnLosers.toFixed(2)}`,
            subtitle: 'Total',
            colorClass: 'text-bloomberg-danger'
        },
        {
            title: 'PF',
            value: journalAnalytics.profitFactor.toFixed(2),
            subtitle: journalAnalytics.profitFactor >= 1 ? 'Good' : 'Bad',
            colorClass: journalAnalytics.profitFactor >= 1 ? 'text-bloomberg-success' : 'text-bloomberg-danger'
        },
        {
            title: 'ROI',
            value: `${journalAnalytics.accumulativeReturnPercent.toFixed(2)}%`,
            subtitle: 'Return',
            colorClass: journalAnalytics.accumulativeReturnPercent >= 0 ? 'text-bloomberg-success' : 'text-bloomberg-danger'
        },
        {
            title: 'Balance',
            value: `$${Number(user?.balance || 0).toFixed(2)}`,
            subtitle: 'MT5 Account',
            colorClass: 'text-bloomberg-accent'
        },
        {
            title: 'Best Trade',
        ...
        },
        {
            ...
            subtitle: 'Per Trade',
            colorClass: journalAnalytics.tradeExpectancy >= 0 ? 'text-bloomberg-success' : 'text-bloomberg-danger'
        },
        {
            title: 'P/L Ratio',
            value: journalAnalytics.profitLossRatio.toFixed(2) + ':1',
            subtitle: `$${journalAnalytics.avgWinAmount.toFixed(2)} / $${Math.abs(journalAnalytics.avgLossAmount).toFixed(2)}`,
            colorClass: 'text-bloomberg-accent'
        }
    ];
        ...
        <div className="max-w-[1920px] mx-auto px-3 py-2 space-y-2">
            ...
            <div className="flex items-center justify-between mb-2">
                <h1 className="text-lg font-bold text-bloomberg-text-primary terminal-text flex items-center space-x-2">
                    ...
                    {refreshing && <span className="text-[10px] text-bloomberg-accent">● Syncing</span>}
                </h1>
                <div className="flex items-center space-x-3 text-[10px]">
                    <div>
                        <span className="text-bloomberg-text-muted">Balance: </span>
                        <span className="text-bloomberg-accent terminal-text font-bold">${Number(user?.balance || 0).toFixed(2)}</span>
                    </div>
                    <div className="text-bloomberg-text-muted">MT5: {user?.accountNumber}</div>
                </div>
            </div>

            {/* High-Density Top Stats Ribbon */}
            <div className="card-bloomberg rounded-lg p-2 relative">
                {/* Navigation Buttons */}
                <button
                    onClick={() => setCurrentStatsIndex(Math.max(0, currentStatsIndex - 4))}
                    disabled={currentStatsIndex === 0}
                    className={`absolute left-1 top-1/2 -translate-y-1/2 z-10 p-1 rounded ${
                        currentStatsIndex === 0 
                            ? 'bg-bloomberg-bg-tertiary text-bloomberg-text-muted cursor-not-allowed'
                            : 'bg-bloomberg-accent text-black hover:bg-bloomberg-accent-dark'
                    }`}
                >
                ...
                <button
                    onClick={() => {
                        const maxIndex = Math.max(0, statsItems.length - 4);
                        setCurrentStatsIndex(Math.min(maxIndex, currentStatsIndex + 4));
                    }}
                    disabled={currentStatsIndex >= statsItems.length - 4}
                    className={`absolute right-1 top-1/2 -translate-y-1/2 z-10 p-1 rounded ${
                        currentStatsIndex >= statsItems.length - 4
                            ? 'bg-bloomberg-bg-tertiary text-bloomberg-text-muted cursor-not-allowed'
                            : 'bg-bloomberg-accent text-black hover:bg-bloomberg-accent-dark'
                    }`}
                >
                ...
                <div className="overflow-hidden px-8">
                    <div className="flex items-center justify-between space-x-2">
                        {statsItems.slice(currentStatsIndex, currentStatsIndex + 4).map((item, index) => (
                            <div key={index} className="flex-1 px-3 py-1.5 border-r border-bloomberg-border last:border-r-0">
                                <div className="text-[9px] text-bloomberg-text-muted uppercase font-semibold">{item.title}</div>
                                <div className={`text-lg font-bold terminal-text mt-0.5 ${item.colorClass}`}>
                                    {item.value}
                                </div>
                                <div className="text-[8px] text-bloomberg-text-muted">{item.subtitle}</div>
                            </div>
                        ))}
                    </div>
                </div>
